...
const promptData = { items: ['The moment relief washes over after weeks of tension', 'Overwhelming grief that comes in unexpected waves', 'The quiet joy of a perfect morning', 'Betrayal from someone trusted completely', 'Pride mixed with bittersweet sadness', 'Crushing disappointment after high hopes', 'The warmth of belonging after feeling alone', 'Paralyzing fear that freezes all thought', 'Unexpected jealousy toward a best friend', 'Contentment in a simple moment', 'Rage barely contained beneath calm exterior', 'Nostalgia for a time that cannot return', 'Anxiety manifesting as physical symptoms', 'Love that terrifies with its intensity', 'Shame that colors everything', 'Wonder at something truly beautiful', 'Frustration at being misunderstood', 'Gratitude that brings tears', 'Loneliness in a crowded room', 'Anticipation before life changes', 'Regret for words that cannot be unsaid', 'Empathy that overwhelms boundaries', 'Confusion between what is felt and shown', 'Numbness after too much emotion', 'Hope despite all evidence against it', 'Guilt that wont fade', 'Serenity hard-won after struggle', 'Excitement tinged with dread', 'Compassion for an enemy', 'Acceptance of the inevitable'] };
...
const RandomEmotionPromptGeneratorPage = () => {
  ...
  const generatePrompt = useCallback(() => {
    ...
    const prompt = { id: Date.now(), text: batchSize === 1 ? items[0] : items.join('\n\n'), category: 'randomemotionpromptgenerator', timestamp: new Date().toISOString(), isMultiple: batchSize > 1 };
  ...
  const sharePrompt = async (prompt) => { if (navigator.share) { try { await navigator.share({ title: 'Random Emotion Prompt Generator', text: prompt.text, url: window.location.href }); } catch (err) { copyToClipboard(`${prompt.text}\n\nGenerated at: ${window.location.href}`); } } else { copyToClipboard(`${prompt.text}\n\nGenerated at: ${window.location.href}`); } };
  const exportPrompts = () => { const dataStr = JSON.stringify(savedPrompts, null, 2); const dataBlob = new Blob([dataStr], { type: 'application/json' }); const url = URL.createObjectURL(dataBlob); const link = document.createElement('a'); link.href = url; link.download = 'saved-randomemotionpromptgenerator.json'; link.click(); URL.revokeObjectURL(url); };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white">
      <SEO pageKey="randomEmotionPromptGenerator" />
      <Header promptHistory={promptHistory} showHistory={showHistory} onHistoryToggle={() => setShowHistory(!showHistory)} />
      <div className="bg-gradient-to-r from-rose-600 to-red-600 text-white py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Random Emotion Prompt Generator</h1>
          <p className="text-lg opacity-80 mb-8 max-w-2xl mx-auto">Generate emotion prompts for character development and emotional depth in writing.</p>
        </div>
      </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mt-16 space-y-8">
            <div className="prose prose-gray max-w-none">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">Random Emotion Prompt Generator</h2>
              <p className="text-gray-700 mb-4">Generate emotion prompts for character development and emotional depth in writing.</p>
            </div>
          </div>
        </div>
...
export default RandomEmotionPromptGeneratorPage;
