...
const RandomHobbyGeneratorPage = () => {
  ...
  const generatePrompt = useCallback(() => {
    ...
    const prompt = { id: Date.now(), text: batchSize === 1 ? items[0] : items.join('\n\n'), category: 'randomhobbygenerator', timestamp: new Date().toISOString(), isMultiple: batchSize > 1 };
  ...
  const sharePrompt = async (prompt) => { if (navigator.share) { try { await navigator.share({ title: 'Random Hobby Generator', text: prompt.text, url: window.location.href }); } catch (err) { copyToClipboard(`${prompt.text}\n\nGenerated at: ${window.location.href}`); } } else { copyToClipboard(`${prompt.text}\n\nGenerated at: ${window.location.href}`); } };
  const exportPrompts = () => { const dataStr = JSON.stringify(savedPrompts, null, 2); const dataBlob = new Blob([dataStr], { type: 'application/json' }); const url = URL.createObjectURL(dataBlob); const link = document.createElement('a'); link.href = url; link.download = 'saved-randomhobbygenerator.json'; link.click(); URL.revokeObjectURL(url); };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white">
      <SEO pageKey="randomHobbyGenerator" />
      ...
      <div className="bg-gradient-to-r from-emerald-600 to-teal-600 text-white py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Random Hobby Generator</h1>
          <p className="text-lg opacity-80 mb-8 max-w-2xl mx-auto">Generate random hobby ideas to discover new interests and activities.</p>
        </div>
      </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mt-16 space-y-8">
            <div className="prose prose-gray max-w-none">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">Random Hobby Generator</h2>
              <p className="text-gray-700 dark:text-gray-300 mb-4">Generate random hobby ideas to discover new interests and activities.</p>
            </div>
          </div>
        </div>
...
export default RandomHobbyGeneratorPage;
