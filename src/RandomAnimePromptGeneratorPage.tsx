...
const promptData = { items: ['Magical girl transformation sequence with sparkles', 'Mecha pilot in cockpit during intense battle', 'School festival scene with cherry blossoms', 'Chibi characters in super deformed style', 'Dark anime villain with dramatic lighting', 'Slice of life cafe scene with friends', 'Fantasy adventure party traveling together', 'Emotional crying scene with dramatic tears', 'Action pose with speed lines and impact', 'Romantic confession under sakura tree', 'Tournament arc intense fighting moment', 'Tsundere character blushing and flustered', 'Isekai protagonist arriving in new world', 'Ninja performing stealth mission at night', 'Idol group performing on stage', 'Mysterious transfer student introduction', 'Beach episode with swimsuit\'s and fun', 'Training montage showing progress', 'Villain monologue with evil laugh', 'Power-up transformation glowing aura', 'Comedic reaction with exaggerated expression', 'Shoujo romance with sparkly effects', 'Battle-damaged hero standing defiant', 'Cooking anime food preparation scene', 'Spirit familiar bonding with protagonist', 'Group photo after adventure completion', 'Dramatic cliffhanger ending moment', 'Tournament bracket reveal scene', 'Friendship power-up moment', 'Final boss intimidating entrance'] };
const weightedRandom = (items) => items[Math.floor(Math.random() * items.length)];

const RandomAnimePromptGeneratorPage = () => {
  const [generatedPrompt, setGeneratedPrompt] = useState(null);
  const [savedPrompts, setSavedPrompts] = useLocalStorage('random-anime-prompt-generator-saved-prompts', []);
  const [promptHistory, setPromptHistory] = useLocalStorage('random-anime-prompt-generator-prompt-history', []);
  const [favorites, setFavorites] = useLocalStorage('random-anime-prompt-generator-favorites', []);
  ...
  const generatePrompt = useCallback(() => {
    ...
    const prompt = { id: Date.now(), text: batchSize === 1 ? items[0] : items.join('\n\n'), category: 'randomanimepromptgenerator', timestamp: new Date().toISOString(), isMultiple: batchSize > 1 };
  ...
  const sharePrompt = async (prompt) => { if (navigator.share) { try { await navigator.share({ title: 'Random Anime Prompt Generator', text: prompt.text, url: window.location.href }); } catch (err) { copyToClipboard(`${prompt.text}\n\nGenerated at: ${window.location.href}`); } } else { copyToClipboard(`${prompt.text}\n\nGenerated at: ${window.location.href}`); } };
  const exportPrompts = () => { const dataStr = JSON.stringify(savedPrompts, null, 2); const dataBlob = new Blob([dataStr], { type: 'application/json' }); const url = URL.createObjectURL(dataBlob); const link = document.createElement('a'); link.href = url; link.download = 'saved-randomanimepromptgenerator.json'; link.click(); URL.revokeObjectURL(url); };
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 shadow-sm transition-colors">
        {isMultiple ? (
          <div className="mb-4 space-y-3">
            {prompt.text.split('\n\n').map((item, index) => (
              <div key={index} className="bg-gray-50 px-4 py-3 rounded border text-gray-800 dark:text-gray-200 leading-relaxed">{item}</div>
            ))}
          </div>
        ) : (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white">
      <SEO pageKey="randomAnimePromptGenerator" />
      ...
      <div className="bg-gradient-to-r from-pink-500 to-rose-500 text-white py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Random Anime Prompt Generator</h1>
          <p className="text-lg opacity-80 mb-8 max-w-2xl mx-auto">Generate anime and manga art prompts with character and scene ideas.</p>
        </div>
      </div>
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex flex-wrap justify-center gap-2 mb-8 border-b border-gray-200 dark:border-gray-700">
          ...
          <Link to="/ai-images-prompt" className="flex items-center gap-2 px-6 py-3 font-medium transition-all text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-700"><Sparkles size={18} /> AI Art</Link>
          <Link to="/short-story-prompts-generator" className="flex items-center gap-2 px-6 py-3 font-medium transition-all text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-700"><Crown size={18} /> Stories</Link>
        </div>
      </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mt-16 space-y-8">
            <div className="prose prose-gray max-w-none">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">Random Anime Prompt Generator</h2>
              <p className="text-gray-700 dark:text-gray-300 mb-4">Generate anime and manga art prompts with character and scene ideas.</p>
            </div>
          </div>
        </div>
...
export default RandomAnimePromptGeneratorPage;
