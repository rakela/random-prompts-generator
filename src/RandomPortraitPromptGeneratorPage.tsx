...
const promptData = { items: ['Wise elderly person with lifetime of stories in eyes', 'Young child with innocent wonder and curiosity', 'Professional in business attire with confident expression', 'Artist with paint-splattered clothe\'s and creative energy', 'Athlete mid-motion showing physical prowess', 'Warrior with battle scars and stern determination', 'Elegant person in formal wear with grace', 'Casual portrait with natural genuine smile', 'Mysterious stranger in shadow with hidden face', 'Emotional close-up showing vulnerability', 'Regal figure in crown and royal bearing', 'Street style fashion portrait with attitude', 'Environmental portrait showing occupation', 'Candid laughter genuine joy moment', 'Contemplative profile against window light', 'Dramatic side lighting highlighting features', 'Multi-generational family group portrait', 'Cultural traditional dress ceremonial portrait', 'Cyberpunk enhanced human with tech implants', 'Fantasy character with magical elements', 'Vintage 1920s style portrait sepia toned', 'Ethereal beauty with soft romantic lighting', 'Intense stare direct eye contact portrait', 'Joyful celebration mid-dance movement', 'Weathered hands telling life story', 'Silhouette against dramatic background', 'Fashion editorial with high contrast', 'Character study showing personality', 'Moody portrait with dramatic shadows', 'Timeless black and white classic portrait'] };
const weightedRandom = (items) => items[Math.floor(Math.random() * items.length)];

const RandomPortraitPromptGeneratorPage = () => {
  const [generatedPrompt, setGeneratedPrompt] = useState(null);
  const [savedPrompts, setSavedPrompts] = useLocalStorage('random-portrait-prompt-generator-saved-prompts', []);
  const [promptHistory, setPromptHistory] = useLocalStorage('random-portrait-prompt-generator-prompt-history', []);
  const [favorites, setFavorites] = useLocalStorage('random-portrait-prompt-generator-favorites', []);
  ...
  const generatePrompt = useCallback(() => {
    ...
    const prompt = { id: Date.now(), text: batchSize === 1 ? items[0] : items.join('\n\n'), category: 'randomportraitpromptgenerator', timestamp: new Date().toISOString(), isMultiple: batchSize > 1 };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white">
      <SEO pageKey="randomPortraitPromptGenerator" />
      ...
      <div className="bg-gradient-to-r from-amber-600 to-orange-600 text-white py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Random Portrait Prompt Generator</h1>
          <p className="text-lg opacity-80 mb-8 max-w-2xl mx-auto">Generate portrait prompts with poses, expressions, and lighting styles.</p>
        </div>
      </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mt-16 space-y-8">
            <div className="prose prose-gray max-w-none">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">Random Portrait Prompt Generator</h2>
              <p className="text-gray-700 dark:text-gray-300 mb-4">Generate portrait prompts with poses, expressions, and lighting styles.</p>
            </div>
          </div>
        </div>
...
export default RandomPortraitPromptGeneratorPage;
