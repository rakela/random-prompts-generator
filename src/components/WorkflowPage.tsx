...
import { Copy, Loader2, CheckCircle, AlertCircle, FileText, CreditCard, Zap, Lock, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function WorkflowPage({ workflow }: WorkflowPageProps) {
  ...
  const renderInput = (input: typeof workflow.inputs[0], index: number) => {
    ...
    const isDisabled = !user; // Disable if not logged in
      ...
      case 'textarea':
        return (
          <textarea
            ...
            disabled={isDisabled}
            rows={6}
            className={`${baseClasses} ${isDisabled ? 'bg-gray-100 cursor-not-allowed' : ''}`}
          />
        );
  ...
  const faqData = [
    ...
    },
    {
      ...
      answer: `Content generation typically takes ${workflow.steps.length * 30}-${workflow.steps.length * 60} seconds, depending on the video length and current API load. Longer videos may take a bit more time to process.`
    },
    {
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-8">
          ...
          <ul className="text-blue-800 space-y-2">
            ...
            <li>• Click "Generate All Content" to process your request</li>
            <li>• Copy the generated content using the copy buttons</li>
            <li>• Switch between tabs to view different sections</li>
          </ul>
        </div>
