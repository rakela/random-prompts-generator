...
const staticPageUrls = [
  ...
  // Kling 3 (top AI video model 2026 — P3 backlog item)
  'kling-3-prompt-generator',
  // Blog
  'blog',
  // Other pages
  'pricing',
...
export async function GET(context) {
  ...
  const staticPages = staticPageUrls.map((url, index) => ({
    url,
    changefreq: url === '' ? 'daily' : url.startsWith('workflows/') || url.startsWith('tools/') || url.startsWith('blog') || url === 'veo-3-prompt-generator' || url === 'gpt-image-2-prompt-generator' || url === 'chatgpt-action-figure-prompt-generator' || url === 'kling-3-prompt-generator' ? 'daily' : 'weekly',
    priority: url === '' ? '1.0' :
              // YouTube Tools and new AI Tools get very high priority (new features)
              url === 'veo-3-prompt-generator' || url === 'gpt-image-2-prompt-generator' || url === 'chatgpt-action-figure-prompt-generator' || url === 'kling-3-prompt-generator' || url.startsWith('workflows/youtube') || url.startsWith('tools/youtube') || url.startsWith('tools/text-to-prompt') || url.startsWith('tools/image-to-prompt') || url.startsWith('tools/video-ai-generation') || url.startsWith('tools/instagram-aesthetic-generator') ? '0.95' :
