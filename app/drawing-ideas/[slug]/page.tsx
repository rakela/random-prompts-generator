...
// Generate static params for all drawing idea generators
export async function generateStaticParams() {
  const drawingGenerators = getGeneratorsByCategory('drawing');
  return drawingGenerators.map((gen) => ({
    slug: gen.slug,
  }));
}
...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const generator = Object.values(generatorConfig).find(
    gen => gen.slug === params.slug && gen.category === 'drawing'
  );
...
export default function DrawingIdeaGeneratorPage({ params }: { params: { slug: string } }) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 dark:from-gray-900 to-white dark:to-gray-800">
      ...
      <nav className="max-w-6xl mx-auto px-4 py-4 text-sm text-gray-600 dark:text-gray-400">
        <div className="flex items-center gap-2">
          ...
          <Link href="/drawing-ideas/" className="hover:underline hover:text-gray-900 dark:hover:text-gray-100">
            Drawing Ideas
          </Link>
          <ChevronRight size={16} />
      ...
      <GeneratorClient generator={generator} category="drawing" />
