...
// Generate static params for all AI art generators
export async function generateStaticParams() {
  const aiArtGenerators = getGeneratorsByCategory('ai-art');
  return aiArtGenerators.map((gen) => ({
    slug: gen.slug,
  }));
}
...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const generator = Object.values(generatorConfig).find(
    gen => gen.slug === params.slug && gen.category === 'ai-art'
  );
...
export default function AIArtGeneratorPage({ params }: { params: { slug: string } }) {
